...
TEST_F(PreTradeCheckUpdateNotionalTest, UpdateIncreaseQuantityBreachesLimit) {
    ...
    auto result = engine.pre_trade_check(replace);
...
TEST_F(PreTradeCheckUpdateNotionalTest, UpdateDecreaseQuantityAlwaysPasses) {
    ...
    engine.on_new_order_single(order);
    engine.on_execution_report(create_ack("ORD001", 300));
...
protected:
    using GrossDelta = GrossDeltaMetric<UnderlyerKey, StaticInstrumentProvider, AllStages>;
    using NetDelta = NetDeltaMetric<UnderlyerKey, StaticInstrumentProvider, AllStages>;
...
TEST_F(PreTradeCheckUpdateDeltaTest, UpdateBreachesNetDelta) {
    ...
    engine.on_new_order_single(order);
    engine.on_execution_report(create_ack("ORD001", 50));
...
protected:
    using GrossDelta = GrossDeltaMetric<UnderlyerKey, StaticInstrumentProvider, AllStages>;
    using NetDelta = NetDeltaMetric<UnderlyerKey, StaticInstrumentProvider, AllStages>;
    using GlobalNotional = GlobalNotionalMetric<StaticInstrumentProvider, AllStages>;
...
TEST_F(PreTradeCheckSingleMetricTest, SingleMetricCheckNewOrder) {
    ...
    auto full_result = engine.pre_trade_check(order);
    ...
    auto gross_result = engine.pre_trade_check_single<GrossDelta>(order);
    ...
    auto net_result = engine.pre_trade_check_single<NetDelta>(order);
    ...
    auto notional_result = engine.pre_trade_check_single<GlobalNotional>(order);
...
TEST_F(PreTradeCheckSingleMetricTest, SingleMetricCheckUpdate) {
    ...
    auto gross_result = engine.pre_trade_check_single<GrossDelta>(replace);
    EXPECT_TRUE(gross_result.would_breach) << "Gross delta update should breach";
    EXPECT_TRUE(gross_result.has_breach(LimitType::GROSS_DELTA));

    // Single metric check for net delta
    auto net_result = engine.pre_trade_check_single<NetDelta>(replace);
...
TEST_F(ComputeUpdateContributionTest, GrossDeltaContribution) {
    ...
    double contribution = GrossDelta::compute_update_contribution(update, existing, &provider);
...
TEST_F(ComputeUpdateContributionTest, NetDeltaContribution) {
    ...
    double contribution = NetDelta::compute_update_contribution(update, existing, &provider);
    EXPECT_DOUBLE_EQ(contribution, -375000.0);
}

TEST_F(ComputeUpdateContributionTest, NotionalContribution) {
    using Notional = GlobalNotionalMetric<StaticInstrumentProvider, AllStages>;
    ...
    double contribution = Notional::compute_update_contribution(update, existing, &provider);
...
TEST_F(ComputeUpdateContributionTest, OrderCountContributionIsZero) {
    ...
    int64_t contribution = OrderCount::compute_update_contribution<StaticInstrumentProvider>(
...
TEST_F(PreTradeCheckUpdateFullFlowTest, CheckBeforeSendingUpdate) {
    ...
    auto check1 = engine.pre_trade_check(replace);
    ...
    auto check2 = engine.pre_trade_check(replace2);
    EXPECT_FALSE(check2.would_breach) << "Smaller update should pass";

    // Step 4: Send the valid update
    engine.on_order_cancel_replace(replace2);

    // Step 5: ACK the update
    engine.on_execution_report(create_replace_ack("ORD001_R", "ORD001", 300));
    EXPECT_DOUBLE_EQ(notional(), 45000.0);
}
...
TEST_F(PreTradeCheckUpdateFullFlowTest, RejectedUpdateDoesNotAffectMetrics) {
    ...
    // NACK the update
    engine.on_order_cancel_reject(create_replace_nack("ORD001_R", "ORD001"));
