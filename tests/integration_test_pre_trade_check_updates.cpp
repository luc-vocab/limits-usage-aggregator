...
protected:
    using GrossDelta = GrossDeltaMetric<UnderlyerKey, InstrumentData, AllStages>;
    using NetDelta = NetDeltaMetric<UnderlyerKey, InstrumentData, AllStages>;
...
protected:
    using GrossDelta = GrossDeltaMetric<UnderlyerKey, InstrumentData, AllStages>;
    using NetDelta = NetDeltaMetric<UnderlyerKey, InstrumentData, AllStages>;
    using GlobalNotional = GlobalNotionalMetric<InstrumentData, AllStages>;
...
TEST_F(PreTradeCheckUpdateOrderCountTest, UpdateDoesNotAffectOrderCount) {
    ...
    auto new_order_result = engine.pre_trade_check(order2, null_inst);
    ...
    auto replace = create_replace("ORD001_R", "ORD001", "AAPL", Side::BID, 150.0, 200);
    engine.on_order_cancel_replace(replace, null_inst);

    auto update_result = engine.pre_trade_check(replace, null_inst);
...
TEST_F(ComputeUpdateContributionTest, GrossDeltaContribution) {
    ...
    double contribution = GrossDelta::compute_update_contribution(update, existing, inst);
...
TEST_F(ComputeUpdateContributionTest, NetDeltaContribution) {
    ...
    double contribution = NetDelta::compute_update_contribution(update, existing, inst);
    EXPECT_DOUBLE_EQ(contribution, -375000.0);
}

TEST_F(ComputeUpdateContributionTest, NotionalContribution) {
    using Notional = GlobalNotionalMetric<InstrumentData, AllStages>;
    ...
    double contribution = Notional::compute_update_contribution(update, existing, inst);
...
TEST_F(ComputeUpdateContributionTest, OrderCountContributionIsZero) {
    ...
    auto inst = provider.get_instrument("AAPL");

    // Order count doesn't change on update
    int64_t contribution = OrderCount::compute_update_contribution(update, existing, inst);
